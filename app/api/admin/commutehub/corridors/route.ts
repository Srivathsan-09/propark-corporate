...
import { getCorridorIntelligence } from "@/lib/services/commuteHubAnalytics";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const corridors = await getCorridorIntelligence(effectiveCampusId);
    return NextResponse.json({ success: true, data: { corridors } });
  } catch (error: any) {
    console.error("CommuteHub Corridors API Error:", error);
    return NextResponse.json(
      { success: false, error: error.message || "Failed to fetch corridor intelligence" },
      { status: 500 }
    );
  }
}
