...
import { getDemandAndDeficit } from "@/lib/services/commuteHubAnalytics";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const demand = await getDemandAndDeficit(effectiveCampusId);
    return NextResponse.json({ success: true, data: { demand } });
  } catch (error: any) {
    console.error("CommuteHub Demand API Error:", error);
    return NextResponse.json(
      { success: false, error: error.message || "Failed to fetch commute demand analytics" },
      { status: 500 }
    );
  }
}
