...
export default function CreateHubPage() {
  ...
  useEffect(() => {
    ...
    if (!isOriginValid && !isDestValid && validIntermediateList.length === 0) {
      setCalculatedRoute(null);
      return;
    }

    const waypointsCount = (isOriginValid ? 1 : 0) + validIntermediateList.length + (isDestValid ? 1 : 0);
    if (waypointsCount < 2) {
      setCalculatedRoute(null);
      return;
    }
      ...
      try {
        ...
        if (isOriginValid && isDestValid) {
          ...
          try {
            ...
            console.warn("Base highway calculation failed:", e);
          }

          // 2. If intermediate hubs exist, snap them to the base highway route polyline
          const snappedIntermediates = validIntermediateList.map((hub) => {
          ...
          validIntermediateList.forEach((hub) => activeWaypoints.push({ latitude: hub.latitude, longitude: hub.longitude }));
    ...
    if (!corridor || corridor.includes("→")) {
      if (isOriginValid && validIntermediateList.length > 0 && isDestValid) {
        setCorridor(`${origin.name} → ${validIntermediateList.map((p) => p.name).join(" → ")} → ${destination.name}`);
      } else if (isOriginValid && isDestValid) {
        setCorridor(`${origin.name} → ${destination.name}`);
      } else if (isOriginValid && validIntermediateList.length > 0) {
        setCorridor(`${origin.name} → ${validIntermediateList.map((p) => p.name).join(" → ")}`);
      }
    }
  ...
  }, [origin, intermediateHubs, destination]);
            ...
            <CardHeader className="p-3.5 pb-2.5 border-b border-slate-100 flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription className="text-[11px] text-slate-500">
                  Set corridor route, intermediate hubs, and campus
                </CardDescription>
              </div>
            </CardHeader>
