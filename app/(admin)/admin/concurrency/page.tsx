...
export default function AdminConcurrencyPage() {
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in-50 duration-300">
      ...
      {/* 1. Architecture Flowchart Visualizer */}
      <Card className="border-slate-200 bg-slate-950 text-white shadow-xl rounded-2xl overflow-hidden">
        <CardHeader className="border-b border-slate-800 pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Layers className="h-4 w-4 text-emerald-400" />
              <CardTitle className="text-sm font-bold text-white">
                Live High-Concurrency Architecture Pipeline
              </CardTitle>
            </div>
            <Badge variant="outline" className="text-emerald-400 border-emerald-500/40 text-[10px]">
              Round-Robin Routing Active
            </Badge>
          </div>
        </CardHeader>
        ...
        <CardContent className="p-4 sm:p-6">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3 text-center text-xs">
            {/* Step 1: Users */}
            <div className="bg-slate-900/90 border border-slate-800 p-3.5 rounded-xl space-y-1.5 flex flex-col justify-center">
              <Users className="h-5 w-5 text-blue-400 mx-auto" />
              <span className="font-extrabold text-white block">COMMUTEX USERS</span>
              <span className="text-[10px] text-slate-400 block">100+ Concurrent Employees</span>
            </div>

            {/* Arrow */}
            <div className="hidden md:flex items-center justify-center text-slate-600">
              <ArrowRight className="h-5 w-5 text-emerald-500 animate-pulse" />
            </div>

            {/* Step 2: Load Balancer */}
            <div className="bg-gradient-to-b from-emerald-950 to-slate-900 border border-emerald-500/40 p-3.5 rounded-xl space-y-1.5">
              <Cpu className="h-5 w-5 text-emerald-400 mx-auto" />
              <span className="font-extrabold text-emerald-300 block">LOAD BALANCER</span>
              <Badge className="bg-emerald-500/20 text-emerald-300 text-[9px] border-emerald-500/30">
                ROUND ROBIN
              </Badge>
              <span className="text-[10px] text-slate-400 block">
                Total Routed: {lb?.totalRequestsRouted || 0}
              </span>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 p-3.5 rounded-xl space-y-1.5">
              ...
              <span className="font-extrabold text-white block">3 BACKEND NODES</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3 text-center text-xs">
            {/* Step 4: Ride FIFO Queues */}
            <div className="bg-slate-900/90 border border-slate-800 p-3.5 rounded-xl space-y-1.5 flex flex-col justify-center">
              <Clock className="h-5 w-5 text-amber-400 mx-auto" />
              <span className="font-extrabold text-white block">RIDE-SCOPED QUEUES</span>
              <span className="text-[10px] text-amber-300 block">
                Isolated per RideID
              </span>
              <span className="text-[10px] text-slate-400 block">
                Waiting: {queues?.totalWaiting || 0}
              </span>
            </div>
            ...
            {/* Step 5: Workers */}
            <div className="bg-slate-900/90 border border-slate-800 p-3.5 rounded-xl space-y-1.5">
              <Activity className="h-5 w-5 text-teal-400 mx-auto" />
              <span className="font-extrabold text-white block">CONCURRENT WORKERS</span>
              <span className="text-[10px] text-teal-300 block">
                {workers?.activeWorkerCount || 4} Parallel Workers
              </span>
              <span className="text-[10px] text-slate-400 block">
                Processed: {workers?.totalProcessed || 0}
              </span>
            </div>
            ...
            {/* Step 6: Atomic MongoDB Atlas */}
            <div className="bg-gradient-to-b from-emerald-950 to-slate-900 border border-emerald-500/40 p-3.5 rounded-xl space-y-1.5">
              <Database className="h-5 w-5 text-emerald-400 mx-auto" />
              <span className="font-extrabold text-emerald-300 block">ATOMIC MONGODB</span>
