...
export default function RegisterPage() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setServerError("Could not initiate Google sign in. Please try again.");
    }
  };
        ...
        {step === "form" ? (
          <div className="space-y-2.5">
            ...
            <Button
              ...
              disabled={isGoogleLoading || isLoading || isSendingOtp}
              className="w-full h-10 border-slate-200 bg-white hover:bg-emerald-50/50 hover:border-emerald-300 font-medium text-slate-700 flex items-center justify-center gap-2.5 transition-colors shadow-xs rounded-xl text-xs"
            >
              {isGoogleLoading ? (
                <div className="flex items-center justify-center gap-2">
                  <Loader2 className="h-3.5 w-3.5 animate-spin text-slate-600" />
                  <span className="font-semibold text-slate-800 text-xs">Connecting to Google...</span>
                </div>
              ) : (
                <div className="flex items-center justify-center gap-2.5">
                  ...
                  <span className="font-semibold text-slate-800 text-xs">Sign Up / Sign In with Google</span>
                </div>
              )}
            </Button>
